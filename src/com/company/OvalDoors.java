...
public class OvalDoors implements Adding {
    ...
    public void draw(Graphics g, int startPosX, int startPosY) {
        ...
        g.fillOval(startPosX + 195, startPosY + 10, 20, 30);
        g.setColor(Color.BLACK);
        g.drawOval(startPosX + 195, startPosY + 10, 20, 30);
        g.drawRect(startPosX + 203, startPosY + 10, 2, 30);
        if (count == DoorsCount.four || count == DoorsCount.five) {
            g.setColor(Color.CYAN);
            g.fillOval(startPosX + 245, startPosY + 10, 20, 30);
            g.setColor(Color.BLACK);
            g.drawOval(startPosX + 245, startPosY + 10, 20, 30);
            g.drawRect(startPosX + 253, startPosY + 10, 2, 30);
        }
        if (count == DoorsCount.five) {
            g.setColor(Color.CYAN);
            g.fillOval(startPosX + 300, startPosY + 10, 20, 30);
            g.setColor(Color.BLACK);
            g.drawOval(startPosX + 300, startPosY + 10, 20, 30);
